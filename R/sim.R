...
rules = list(
	...
	dimension = 3, #links per node; does the model use links, triangles, or tetrahedra?
...
Sim = function(init,rules){
	...
	sim$chunks = Chunk(sim$points, rules$link_range * 18)
...
grow = function(sim, nid){
	...
	#chunking just isn't worth it lmfao
	#vectorization calcs are cheaper than all this shuffling
	...
	if (length(closest_point) && dists[closest_point] < sim$rules$link_range) {#if a closest valid point exists to link to
		...
		for (p in 1:sim$rules$dimension){ #add number of nodes equal to dimension
			...
			sim$nodes[sim$nid,] = c(pts, 100, node[5] + 1, nid, 0, 0, 0, 1, 0, 0, 0) #pos will get updated during contraction #ADD MOBILITY HERE
			sim$nid = sim$nid + 1
		}
	}
...
contract = function(sim){
	...
	glob_velocity = (glob_avg - glob_points) * 0.0001 #CONTRACTION
...
network3d = function(sim){
	real_nids = which(sim$nodes[,1] > 0)
	n_real = length(real_nids)
	plot3d(NULL,xlim=c(0,sim$init$length),ylim=c(0,sim$init$width))
	for (n in 1:n_real){
		node = sim$nodes[n,]
		pnode = sim$nodes[node[6],]
		lines3d(c(node[11],pnode[11]),c(node[12],pnode[12]),c(node[13],pnode[13]))
	}
}

sim=Sim(init,rules)
if (1) for(i in 1:1000){
	cat('\r',i,'     ')
	sim=tick(sim)
}
network3d(sim)
